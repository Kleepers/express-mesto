...
const { validationError, serverError, castError } = require('../utils/constants')
    ...
    Card.findByIdAndDelete(cardId)
        ...
        .catch((err) => {
            if (err.name === 'CastError') {
                res.status(castError).send({ message: 'Карточка не найдена'})
            }
            else {
    ...
    Card.findByIdAndUpdate(cardId, {$pull: { likes: req.user._id} }, {new: true})
        ...
        .catch((err) => {
            if (err.name === 'CastError') {
                ...
                res.status(serverError).send({ message: 'На сервере произошла ошибка при дизлайке карточки'})
            }
        });
}
